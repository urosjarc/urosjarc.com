...
describe('Log in as UCENEC and test navbar links', () => {
  ...
  it('loop over objects in the array', () => {
// Loop through each object in kontakt_refs array
    ucenecResponse.kontakt_refs.forEach((item: any) => {
      const kontakt = item.kontakt;

      // Check if "tip" is "EMAIL" or "TELEFON" and extract the value accordingly
      let contactValue;
      if (kontakt.tip === "EMAIL") {
        contactValue = kontakt.data;
      } else if (kontakt.tip === "TELEFON") {
        contactValue = kontakt.data;
      }

      // Do something with the extracted value (contactValue)
